...
Go, 유망한 잘 대답할 수 있는 위주로



상황이 생겨서 이런 이슈로 개발하게 되었다.
요구사항에 맞게 개발하게 되었습니다.

결론은: 변화에 대해서 능동적으로 대처 할 수 있습니다.


패턴 -> 디자인 패턴을 공부해서 싱글 통 패턴이나, 다양한 패턴에 대해서 관심이 있고, 패턴에 대한 사용 이유 준비

성능과 안정성 -> 고객의 요구사항에 맞추면서 안정성을 타협하는 성능을 위한 노력 뉘앙스 준비,,,



테스트 코드, 코드 커버리지, 메소드 커버리지,
클래스 커버리지,

브런치 커버리지 -> 구조에서 테스트 하기 위해서 mock 테스트나, tool로 소나큐브를 이용해서 어떻게 적용했는지;;;;;;;

브랜치 커버리지 -> ??????


JPA, mybatis 차이점 기준점 -> 조인 관계


과거는 현재일을 잘 하기 위한 과정일뿐
---
...
# SonarQube
* 정적 코드 분석 툴 (프로그램 실행 없이 코드 분석)
* 코드를 분석하여 중복, 테스트 커버리지, 코드 복잡도, 버그 등을 리포팅
* IDE 통합하여 사용 가능
* 정적 분석은 소스 코드의 모든 부분을 확인할 수 있지만, 실행 환경에서의 상태를 정확히 알 수 없는 단점도 존재 -> 동적 분석
* 정적 분석 도구: 개발 단계에서 소스 코드 분석
* 동적 분석 도구: 테스트, 모니터링
* docker 설치
* 젠킨스, 깃과 연동하여 CI/CD 관리도 가능

## 코트 커버리지
* 코드 커버리지란?
  + 테스트 케이스가 얼마나 충족되었는지 나타내는 지표
  + 코드 자체가 얼마나 실행되었느냐
* 코드 커버리지는 화이트 박스 테스트로 측정
  + 응용 프로그램 내부 구조 동작, 소스코드를 검사하는 테스트 방식
  + 개발자 관점의 단위 테스트 방식
  + 주로 구문 커버리지 사용
* 코드 커버리지 측정 기준 3가지
  + 구문(라인): 코드가 한번이상 실행 되면 충족
    - (수행된 구문의 수 / 전체 구문의 수) * 100
  + 조건: 모든 조건식의 조건이 true/false 중 테스트에 실행된 정도 측정
    - (수행된 조건의 수 / 전체 조건의 수) * 100
  + 결정(브랜치): 전체 if문 같은 분기 중 테스트 의해 실행된 정도 측정
    - (수행된 분기 수 / 전체 분기 수) * 100
* 코드 커버러지의 중요성
  + 테스트 코드의 중요성과 일맥상통
  + 휴먼 에러 방지
  + 코드 커버러지가 기존보다 떨어지는 경우 커밋 제한 기능

---

# Docker
* 컨테이너 기술, 가상머신 처럼 모든 하드웨어를 전부 구현하지 않기 때문에, 매우 빠른 실행 가능
* 컨테이너를 격리하는 기술
  + 리눅스 네임 스페이스
  + 컨트롤 그룹
* 서비스가 커지면 커질수록 관리해야 하는 컨테이너 양이 증가
* 모든 서버를 동일하게 관리하기 위해 k8s 사용
...
![](../../devops/Kubernetes/assets/README-d61d4db7.png)
